...
void calculate_pagerank(double pagerank[])
{
    ...
    double local_pagerank[GRAPH_ORDER];
    ...
    int rows_per_process = GRAPH_ORDER / size;
    
    // Find start and end row for each process
    int start_row = rank * rows_per_process;
    int end_row = (rank == size - 1) ? GRAPH_ORDER : (rank + 1) * rows_per_process;
    ...
    while(elapsed < MAX_TIME && (elapsed + time_per_iteration) < MAX_TIME)
    {
        ...
        for(int i = start_row; i < end_row; i++)
        {
            for(int j = 0; j < GRAPH_ORDER; j++)
            {
                if (adjacency_matrix[j][i] == 1.0)
                {
                    ...
                    local_pagerank[i] += pagerank[j] / (double)outdegree;
                }
            }
        }
        ...
        MPI_Allreduce(local_pagerank, new_pagerank, GRAPH_ORDER, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
